          ...
          <div className="space-y-8">
            {toolCategories.map((category, index) => (
              <div key={index} className="card-cyber p-6">
                <h3 className="terminal-text text-lg font-semibold text-primary mb-4">
                  {category.category}
                </h3>
                <div className="flex flex-wrap gap-3">
                  {category.tools.map((tool, toolIndex) => (
                    <div
                      key={toolIndex}
                      className="px-4 py-2 bg-background border border-border rounded-lg text-sm text-foreground hover:border-primary/50 transition-colors cursor-default"
                    >
                      {tool}
                    </div>
                  ))}
